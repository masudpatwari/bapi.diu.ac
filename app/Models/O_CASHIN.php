...
//		if($preSemesterDue > $currentDues ) $preSemesterDue = $currentDues;
    ...
    public static function  get_student_account_info_summary_by_reg( $reg_no ){


   

         $student = O_STUDENT::with('department')->selectRaw("ID ,  NAME ,  ROLL_NO ,  REG_CODE ,  PASSWORD ,  DEPARTMENT_ID ,  BATCH_ID, ACTUAL_FEE , NO_OF_SEMESTER, payment_from_semester")->where('reg_code','like','%'. $reg_no.'%' )->first();
        // return  $student;

             $cashinCollection = self::where(['student_id' => $student->id])
            //->where('is_varified', self::IS_VARIFIED) // no need to apply because, all account payment is not verified .
            ->with('purposePay')
            ->get();
